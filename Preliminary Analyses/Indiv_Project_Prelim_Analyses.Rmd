...
p1 <- ggplot(data= OREI_2016, aes(x = compost, y = DOC))+
  ...
  labs (title = 'One Year After \ Compost Application', 
        x = "Compost Rate (Mg/ha)", y = "Dissolved Organic Carbon (mg/kg)")
...
library(patchwork)
p1 + p2 + scale_y_continuous(limits = c(0, 300)) 
```

#Things I Changed Based on Feedback
X-included a table of contents
X-included a bolded goals statement
-copy and paste code where I run code again
-make axes the same for last graphs
-use faceting to make lots of graphs, with R2?
-overlaying graphs on top of each other, and then color
X-note what DOC means!
X-put graph titles into two lines with /
-summary after graph
-There were a few times where you make statements such as with outliers and some variables have non normal residuals. I would suggest maybe adding a line of code to show how you came to those conclusions ie. Which(p.vals.trans. <= 0.05). 
-The titles are a bit long, remember you can use \n to break up your text into multiple lines. 
-since you have the same information on both y axes you may be able to remove the y label from the second graph. 
